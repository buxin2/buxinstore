...
def configure_logging(app: Flask) -> None:
    """Send logs to stdout with sane defaults for serverless deployments."""
...
def register_request_hooks(app: Flask) -> None:
    """Attach request/response middleware for logging and caching."""
    ...
    def log_request_start():
        ...
        app.logger.debug(
            "➡️ %s %s user=%s", request.method, request.full_path, user_id or "anon"
        )
        return None
...
from .config import Config, DEFAULT_SQLITE_PATH
from .extensions import db, login_manager, migrate, init_extensions, mail
...
def create_app():
    ...
    # Ensure local SQLite fallback path exists
    ...
    app.config["MAIL_USE_TLS"] = (
        str(os.environ.get("MAIL_USE_TLS", "True")).lower() == "true"
    )
    ...
    app.config["PROFILE_PICTURE_FOLDER"] = os.path.join(
        app.config["UPLOAD_FOLDER"], "profile_pictures"
    )
    app.config["PROFILE_PICTURE_MAX_SIZE"] = 5 * 1024 * 1024
    ...
    app.config.setdefault("STATIC_CACHE_MAX_AGE", 31536000)

    @app.route("/static/<path:filename>")
    def serve_static(filename):
        response = send_from_directory(app.static_folder, filename)
        response.headers["Cache-Control"] = (
            f"public, max-age={app.config['STATIC_CACHE_MAX_AGE']}, immutable"
        )
        response.headers.setdefault("X-Content-Type-Options", "nosniff")
        return response
...
app.config.setdefault('WTF_CSRF_SECRET_KEY', os.environ.get('WTF_CSRF_SECRET_KEY', 'replace-me'))
...
# Login manager is already initialized in init_extensions()
    ...
    if request.method == 'POST':
        ...
        if user:
            try:
                ...
                msg = Message(
                    subject='Reset Your Password - BuXin Store',
...
def admin_settings_backup_database():
    ...
    try:
        ...
        # Get the actual database path - SQLAlchemy resolves relative paths
        # Get the engine's URL to see what SQLAlchemy actually uses
        engine = db.engine
        actual_uri = str(engine.url)
        
        # Extract the actual path from the engine URL
        if 'sqlite:///' in actual_uri:
            db_path = actual_uri.replace('sqlite:///', '', 1)
        elif 'sqlite://' in actual_uri:
            db_path = actual_uri.replace('sqlite://', '', 1)
        
        # If still relative, check common locations
        if not os.path.isabs(db_path):
            # Get Flask instance path
            instance_folder = app.instance_path if hasattr(app, 'instance_path') else os.path.join(app.root_path, 'instance')
            
            # Try different possible locations
            possible_paths = [
                os.path.join(instance_folder, db_path),  # instance/store.db
                os.path.join(instance_folder, os.path.basename(db_path)),  # instance/store.db (basename)
                os.path.join(app.root_path, db_path),  # root/store.db
                os.path.join(app.root_path, 'instance', db_path),  # root/instance/store.db
                os.path.join(app.root_path, 'instance', os.path.basename(db_path)),  # root/instance/store.db (basename)
            ...
            if not found:
                ...
                current_app.logger.error(f"Database file not found. Attempted paths: {attempted_paths}")
                return jsonify({
                    'success': False, 
                    'message': f'Database file not found. Searched in: {", ".join(attempted_paths)}'
                }), 400
        else:
            # Absolute path - verify it exists
            if not os.path.exists(db_path):
                current_app.logger.error(f"Database file not found at absolute path: {db_path}")
                return jsonify({
                    'success': False,
...
def test_email_config():
    """Test email configuration and send a test message (default admin@example.com)."""
    from .extensions import mail as _mail
    from flask_mail import Message
    import smtplib
    
    # Get configuration from environment and app config
    env_config = {
        'MAIL_SERVER': os.environ.get('MAIL_SERVER', 'Not set in env'),
        'MAIL_PORT': os.environ.get('MAIL_PORT', 'Not set in env'),
        'MAIL_USE_TLS': os.environ.get('MAIL_USE_TLS', 'Not set in env'),
        'MAIL_USERNAME': os.environ.get('MAIL_USERNAME', 'Not set in env'),
        'MAIL_PASSWORD': '***' if os.environ.get('MAIL_PASSWORD') else 'Not set in env',
    }
    ...
    app_config = {
        ...
        'Mail Initialized': 'Yes' if _mail else 'No'
    }
    
    # Test SMTP connection
    smtp_test_result = None
    if request.method == 'GET' and request.args.get('test_smtp') == 'true':
        try:
            ...
            server.quit()
            smtp_test_result = "✅ SMTP connection successful!"
            app.logger.info("✅ SMTP connection test passed")
        except Exception as e:
            smtp_test_result = f"❌ SMTP connection failed: {str(e)}"
            app.logger.error(f"❌ SMTP connection test failed: {str(e)}")
    
    if request.method == 'POST':
        test_email = request.form.get('test_email', 'admin@example.com')
        try:
            ...
            msg = Message(
                ...
                recipients=[test_email],
                sender=app.config.get('MAIL_DEFAULT_SENDER', app.config.get('MAIL_USERNAME')),
                body='''This is a test email from your BuXin Store application.
...
def quote_identifier(identifier: str) -> str:
    """Safely quote table/column names for cross-database compatibility."""
    if not identifier:
        return '""'
...
def is_sqlite_backend() -> bool:
    """Detect whether the active SQLAlchemy engine is using SQLite."""
    try:
...
def get_database_path():
    """Get the actual database file path from SQLAlchemy URI when using SQLite."""
...
def _export_sql_dump(db_path: str, destination: str):
    if not is_sqlite_backend():
        raise RuntimeError("SQL dump export is only supported when using the SQLite fallback database.")

    conn = sqlite3.connect(db_path)
    try:
        with open(destination, 'w', encoding='utf-8') as dump_file:
...
def _send_backup_success_email(recipient: Optional[str], db_backup_path: str, sql_backup_path: str, timestamp: datetime):
    if not recipient:
    ...
    msg = Message(
        ...
        body="Your automated daily database backup is ready.\nAttached are the .db backup and the SQL dump."
    )
    attachments = [
        (db_backup_path, 'application/octet-stream'),
        (sql_backup_path, 'application/sql')
    ]
    for path, mime in attachments:
        with open(path, 'rb') as fh:
            msg.attach(os.path.basename(path), mime, fh.read())
    mail.send(msg)
    ...
    try:
        with app.app_context():
            ...
            if not db_path or not os.path.exists(db_path):
                raise FileNotFoundError('Database file not found for backup')
            backup_dir = get_backup_directory()
            timestamp = datetime.utcnow()
            date_str = timestamp.strftime('%Y_%m_%d')
            base_name = f'backup_{date_str}'
            db_backup_path = os.path.join(backup_dir, f'{base_name}.db')
            sql_backup_path = os.path.join(backup_dir, f'{base_name}.sql')
            suffix = 1
            while os.path.exists(db_backup_path) or os.path.exists(sql_backup_path):
                db_backup_path = os.path.join(backup_dir, f'{base_name}_{suffix}.db')
                sql_backup_path = os.path.join(backup_dir, f'{base_name}_{suffix}.sql')
                suffix += 1

            shutil.copy2(db_path, db_backup_path)
            _export_sql_dump(db_path, sql_backup_path)
            ...
            try:
                _send_backup_success_email(recipient, db_backup_path, sql_backup_path, timestamp)
            ...
            log_entry = DatabaseBackupLog(
                status='success',
                file_paths=json.dumps({'db': db_backup_path, 'sql': sql_backup_path}),
                trigger=trigger,
            ...
            current_app.logger.info(f"Database backup completed ({db_backup_path}, {sql_backup_path})")
            return {
                'success': True,
                'message': 'Backup completed and emailed successfully.',
                'db_backup': db_backup_path,
                'sql_backup': sql_backup_path
            }
    except Exception as exc:
        with app.app_context():
            db.session.rollback()
            try:
                ...
                file_map = {}
                if db_backup_path:
                    file_map['db'] = db_backup_path
                if sql_backup_path:
                    file_map['sql'] = sql_backup_path
                log_entry = DatabaseBackupLog(
                    status='fail',
...
def initialize_backup_scheduler():
    ...
    if not is_sqlite_backend():
        app.logger.info("Skipping backup scheduler because PostgreSQL/remote DB is in use.")
        return
    ...
    try:
        ...
        if search:
            ...
            for i, col in enumerate(columns):
                ...
                search_conditions.append(f"CAST({quote_identifier(col)} AS TEXT) LIKE :{param_name}")
        ...
        if sort_column and sort_column in columns:
            query += f" ORDER BY {quote_identifier(sort_column)} {sort_order.upper()}"
        elif primary_keys:
            query += f" ORDER BY {quote_identifier(primary_keys[0])} ASC"
            ...
            try:
                ...
                for col in columns:
                    ...
                    if value is not None and value != '':
                        column_names.append(quote_identifier(col_name))
                ...
                query = f"INSERT INTO {quoted_table} ({', '.join(column_names)}) VALUES ({', '.join(values)})"
...
def admin_database_edit_row(table_name):
    ...
    try:
        ...
        quoted_table = quote_identifier(table_name)
        pk_identifier = quote_identifier(pk_col)
        where_clause = f"{pk_identifier} = :pk_value"
        ...
        if request.method == 'POST':
            try:
                ...
                for col in columns:
                    ...
                    value = request.form.get(col_name)
                    updates.append(f"{quote_identifier(col_name)} = :{col_name}")
                    # Convert value based on type
                    col_type = str(col['type']).upper()
                    if value == '':
                        if col.get('nullable', True):
                            ...
                            # Re-fetch row data
                            with db.engine.connect() as conn:
                                result = conn.execute(text(f"SELECT * FROM {quoted_table} WHERE {where_clause}"), params)
                                row = result.fetchone()
                                if row:
                                    row_data = {}
                                    for i, col in enumerate(columns):
                ...
                query = f"UPDATE {quoted_table} SET {', '.join(updates)} WHERE {where_clause}"
...
def admin_database_delete_row(table_name):
    ...
    try:
        ...
        pk_col = primary_keys[0]
        quoted_table = quote_identifier(table_name)
        query = f"DELETE FROM {quoted_table} WHERE {quote_identifier(pk_col)} = :pk_value"
        params = {'pk_value': row_id}
...
def admin_database_delete_multiple(table_name):
    ...
    try:
        ...
        quoted_table = quote_identifier(table_name)
        query = f"DELETE FROM {quoted_table} WHERE {quote_identifier(pk_col)} IN ({placeholders})"
...
def admin_database_clear_table(table_name):
    ...
    try:
        ...
        if table_name == 'user':
            ...
            query = f"DELETE FROM {quote_identifier('user')} WHERE (COALESCE(is_admin, 0) != 1) AND (COALESCE(role, '') != 'admin')"
        else:
...
def admin_database_export_db():
    ...
    try:
        if not is_sqlite_backend():
            flash('Full-file export is available only when using the local SQLite fallback. For PostgreSQL/Neon, use schema/data SQL exports or pg_dump.', 'warning')
            return redirect(url_for('admin_database_manager'))
...
def admin_database_export_sql():
    ...
    try:
        if not is_sqlite_backend():
            flash('SQL dump export is only available for the local SQLite fallback. Use the generated schema/data SQL files for PostgreSQL.', 'warning')
            return redirect(url_for('admin_database_manager'))
        ...
        return send_file(output, as_attachment=True, download_name=filename, mimetype='text/sql')
    except Exception as e:
...
def admin_database_import_db():
    ...
    try:
        if not is_sqlite_backend():
            flash('Direct .db imports are disabled when using PostgreSQL/Neon. Use psql or pg_restore instead.', 'warning')
            return redirect(url_for('admin_database_manager'))
        ...
        temp_path = os.path.join(app.config['UPLOAD_FOLDER'], f'temp_import_{datetime.now().strftime("%Y%m%d_%H%M%S")}.db')
        ...
        log_database_action('IMPORT', None, None, f"Imported database from {file.filename}")
        flash('Database imported successfully. Backup saved.', 'success')
        return redirect(url_for('admin_database_manager'))
    except Exception as e:
...
def admin_database_import_sql():
    ...
    try:
        if not is_sqlite_backend():
            flash('SQL dump imports are only supported for the local SQLite fallback. Use `psql` or a migration tool for PostgreSQL.', 'warning')
            return redirect(url_for('admin_database_manager'))
        ...
        # Also handle INSERT statements to use INSERT OR REPLACE
        ...
        def replace_index(match):
            unique_part = match.group(1) if match.group(1) else ""
            index_name = match.group(2)
            # Check if IF NOT EXISTS already exists
            if 'IF NOT EXISTS' not in match.group(0).upper():
                return f'CREATE {unique_part}INDEX IF NOT EXISTS {index_name}'
            return match.group(0)  # Return as-is if IF NOT EXISTS already present
        ...
        # Handle INSERT statements - replace with INSERT OR REPLACE
        # But only if it's not already INSERT OR REPLACE or INSERT OR IGNORE
        processed_sql = re.sub(
            r'INSERT\s+(?!OR\s+(?:REPLACE|IGNORE|ROLLBACK|ABORT|FAIL))\s*INTO\s+',
        ...
        for statement in statements:
            try:
                ...
                if 'already exists' not in error_msg.lower():
                    ...
                    current_app.logger.warning(f"SQL statement error: {error_msg}")
        ...
        # Provide feedback about execution
        if errors:
            current_app.logger.warning(f"SQL import completed with {len(errors)} warnings")
        
        log_database_action('IMPORT', None, None, f"Imported SQL dump from {file.filename} - {executed_count} statements executed")
        
        if errors:
            flash(f'SQL dump imported with warnings. {executed_count} statements executed. Some errors were ignored (e.g., tables already exist).', 'warning')
        else:
            flash(f'SQL dump imported successfully. {executed_count} statements executed. Backup saved.', 'success')
        return redirect(url_for('admin_database_manager'))
    except Exception as e:
        current_app.logger.error(f"Error importing SQL: {str(e)}")
        flash(f'Error importing SQL: {str(e)}', 'error')
        return redirect(url_for('admin_database_manager'))
...
def admin_database_backup():
    ...
    try:
        ...
        log_database_action('BACKUP', None, None, f"Created backup: {backup_filename}")
...
def admin_database_restore():
    ...
    try:
        ...
        log_database_action('RESTORE', None, None, f"Restored from backup: {backup_filename}")
        flash(f'Database restored from {backup_filename}. Previous database backed up.', 'success')
        return redirect(url_for('admin_database_manager'))
    except Exception as e:
...
with app.app_context():
    ...
    # Set up relationships with explicit foreign keys and backrefs
    # Note: Email-related relationships have been removed
    ...
    try:
        ...
        if 'hero_image_path' not in columns:
            db.session.execute(text('ALTER TABLE site_settings ADD COLUMN hero_image_path VARCHAR(255)'))
            db.session.commit()
        ...
        for column_name, column_def in backup_column_definitions.items():
            if column_name not in app_settings_columns:
                db.session.execute(text(f"ALTER TABLE app_settings ADD COLUMN {column_name} {column_def}"))
                db.session.commit()
    except Exception as e:
        current_app.logger.error(f"Error ensuring database columns: {str(e)}")
